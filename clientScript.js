	...
	$.getJSON("getSession.php", function(data){
		...
		switch(page)
		{
			case "Main":
				...
				$("#helpText").html("Hier befindest du dich auf der Startseite des Spiels!<br>Du kannst eine Auflistung deiner Basen sehen und die momentanen Ressourcen deiner Anfangsbasis.<br>Du kannst von hier aus zum Schiffcenter, oder der Universumsansicht navigieren.<br><br>Bitte betätige nicht den Zurück-Button deines Browsers!<br>");
			...
			case "Shipcenter":
				...
				inner += '<td align="center"><button class="Button" id="recycleShip" style="width: 150px;" onclick="recycleShip('+base[0].attributes.planet_id.value+')"> Schiff recyceln </button></td></tr>';
				inner += '<tr><td align="center"><button class="Button" id="retrieveShip" style="width: 150px;" onclick="selectShipsToRetrieve()"> Ressourcen abladen </button></td>';
				inner += '<td align="center"><button class="Button" id="giveShip" style="width: 150px;" onclick="giveShip()"> Schiff abgeben </button></td></tr></table>';
				inner += '</div> </div>';
			...
			case "Schiffsbau":	
				...
				inner += '<select name="shiptypes" id="shipSel"></select>';
				inner += '<br><br>';
				...
				inner += '<br>';
				inner += '<span id="NedRes"> <input class=\"resourceInput\" id=\"nedWald\" readonly=\"readonly\" value=\"-\" /> W, ';
				inner += '<input class=\"resourceInput\" id=\"nedHelg\" readonly=\"readonly\" value=\"-\" /> H, '
				inner += '<input class=\"resourceInput\" id=\"nedFran\" readonly=\"readonly\" value=\"-\" /> F, '
				inner += '<input class=\"resourceInput\" id=\"nedMich\" readonly=\"readonly\" value=\"-\" /> M, '
				inner += '<input class=\"resourceInput\" id=\"nedJirk\" readonly=\"readonly\" value=\"-\" /> J, '
				inner += '<input class=\"resourceInput\" di=\"nedThom\" readonly=\"readonly\" value=\"-\" /> T </span>';
				inner += '<br><br>';
				...
				inner += '<br>';
				inner += '<input class="Button" type="submit" value="Schiff Bauen" onclick="buildShip('+base[0].attributes.id.value+')"/>';
				inner += '</div> </div>';
				mainContent.innerHTML = inner; 
				...
				document.getElementById("shipSel").addEventListener("change",needed,false);
			...
			case 'Alliances':
				var politics = xmlDoc.getElementsByTagName("politics");
				var inner = '<div class="subtitle"> Allianz </div>';
				inner += '<div class="text"> <div id="contents">';
				
				if(typeof(politics)==='undefined')
					inner += "Du hast keine Freunden :(";
				else
				{
					inner += "<table width=\"100%\">";
					inner += "<tr><th class=\"shipcenterHeader\">Spieler ID</th><th class=\"shipcenterHeader\">Status</th><th class=\"shipcenterHeader\">Auswahl</th></tr>"
					var politicList = politics[0].getElementsByTagName("politic");
					for(var i = 0; i < politicList.length; i++)
					{
						var politic = politicList[i];
						inner += '<tr><td class="shipcenterHeader">Spieler '+politic.attributes.player_id.value+'</td>';
						
						var stat = "";
						switch(politic.attributes.status.value)
						{
							case '1':
								stat = "Freund";
								break;
						...
						inner += '<td class="shipcenterHeader">'+stat+'</td>';
						inner += '<td class="shipcenterHeader"><input type="radio" name="alliances" title="Spieler auswahl" id="idPlayer_' + politic.attributes.player_id.value + '"/></td></tr>'
					...
					inner += '<br> <br>  <!-- Ich bin ein "aheuaheuaheuaheuaheu" 2 mal :D -->';
					inner += '<table width="100%" style="background-color: rgba(0,0,0,0);"><tr><td align="center"><button class="Button" id="makeFriend" style="width: 150px;" onclick="makeFriend()"> Anfreunden </button></td>';
					inner += '<td align="center"><button class="Button" id="sendShip" style="width: 150px;" onclick="makeEnemy()"> Befeinden</button></td></tr>';
					inner += '<tr><td align="center"><button class="Button" id="mineShip" style="width: 150px;" onclick="makeNeutral()"> Neutral setzen</button></td></tr></table>';
				}
				inner += '</div> </div>';
				mainContent.innerHTML = inner;
				
				$("#helpText").html("Hier befindest du dich im Allianz-Bereich. Du kannst den Status anderer Spieler hier auf befreundet, verfeindet oder neutral setzen.<br>Wähle zuerst einen Spieler aus und klicke dann auf die gewünschte Aktion.<br>");
...
{
	var code = "<input class=\"resourceInput\" id=\"avWald\" readonly=\"readonly\" value=\"" + waldemarium  + "\" />" + " W, ";
	code += "<input class=\"resourceInput\" id=\"avHelg\" readonly=\"readonly\" value=\"" + helgen  + "\" />" + " H, ";
	code += "<input class=\"resourceInput\" id=\"avFran\" readonly=\"readonly\" value=\"" + frankur  + "\" />" + " F, ";
	code += "<input class=\"resourceInput\" id=\"avMich\" readonly=\"readonly\" value=\"" + michaelum  + "\" />" + " M, ";
	code += "<input class=\"resourceInput\" id=\"avJirk\" readonly=\"readonly\" value=\"" + jirkan  + "\" />" + " J, ";
	code += "<input class=\"resourceInput\" id=\"avThom\" readonly=\"readonly\" value=\"" + thomasium  + "\" />" + " T ";
...
function fillNeededResources(number)
{
	...
	{
		code += '<input class="resourceInput" id="nedWald" readonly="readonly" value="' + neededWaldemarium + '" style="color: #ff0000;" />' + ' W, ';
		waldemariumCheck = false;
	}
	else
	{
		code += '<input class="resourceInput" id="nedWald" readonly="readonly" value="' + neededWaldemarium + '"/>' + ' W, ';
		waldemariumCheck = true;
	}
	
	if((neededHelgen - helgen) > 0) 
	{ 
		code += '<input class="resourceInput" id="nedHelg" readonly="readonly" value="' + neededHelgen + '" style="color: #ff0000;" />' + ' H, ';
		helgenCheck = false;
	}
	else
	{
		code += '<input class="resourceInput" id="nedHelg" readonly="readonly" value="' + neededHelgen+ '" />' + ' H, ';
		helgenCheck = true;
	}
	
	if((neededFrankur - frankur) > 0) 
	{
		code += '<input class="resourceInput" id="nedFran" readonly="readonly" value="' + neededFrankur + '" style="color: #ff0000;" />' + ' F, ';
		frankurCheck = false;
	}
	else
	{ 
		code += '<input class="resourceInput" id="nedFran" readonly="readonly" value="' + neededFrankur + '" />' + ' F, ';
		frankurCheck = true;
	}
	
	if((neededMichaelum - michaelum) > 0) 
	{
		code += '<input class="resourceInput" id="nedMich" readonly="readonly" value="' + neededMichaelum + '" style="color: #ff0000;" />' + ' M, ';
		michaelumCheck = false;
	}
	else
	{
		code += '<input class="resourceInput" id="nedMich" readonly="readonly" value="' + neededMichaelum+ '" />' + ' M, ';
		michaelumCheck = true;
	}
	
	if((neededJirkan - jirkan) > 0) 
	{
		code += '<input class="resourceInput" id="nedJirk" readonly="readonly" value="' + neededJirkan + '" style="color: #ff0000;" />' + ' J, ';
		jirkanCheck = false;
	}
	else
	{ 
		code += '<input class="resourceInput" id="nedJirk" readonly="readonly" value="' + neededJirkan + '" />' + ' J, ';
		jirkanCheck = true;
	}
	
	if((neededThomasium - thomasium) > 0) 
	{
		code += '<input class="resourceInput" id="nedThom" readonly="readonly" value="' + neededThomasium + '" style="color: #ff0000;" />' + ' T, ';
		thomasiumCheck = false;
	}
	else
	{ 
		code += '<input class="resourceInput" id="nedThom" readonly="readonly" value="' + neededThomasium + '"/>' + ' T, ';
		thomasiumCheck = true;
	}
...
function recycleShip(baseID)
{
	...
	if(selected.attributes.planet_id.value == baseID)
	{
		$.post("recycle.php", { 'id': selected.attributes.id.value } );
		alert('Schiff wird recyceld!');
		fillHTML("Shipcenter");
	}
	else
	{
		alert('Schiff muss sich an der basis befinden!');
		return;
	}
...
function abbauenChange()
{
	...
	if(document.getElementById("baseAbbauenCheck") != null)
	{
...
function buildShip(baseID)
{

	var type = document.getElementById("typPHP").value;
	var avWald = parseInt(document.getElementById("avWald").value);
	var nedWald = parseInt(document.getElementById("nedWald").value);
	var avHelg = parseInt(document.getElementById("avHelg").value);
	var nedHelg = parseInt(document.getElementById("nedHelg").value);
	var avFran = parseInt(document.getElementById("avFran").value);
	var nedFran = parseInt(document.getElementById("nedFran").value);
	var avMich = parseInt(document.getElementById("avMich").value);
	var nedMich = parseInt(document.getElementById("nedMich").value);
	var avJirk = parseInt(document.getElementById("avJirk").value);
	var nedJirk = parseInt(document.getElementById("nedJirk").value);
	var avThom = parseInt(document.getElementById("avThom").value)
	var nedThom = parseInt(document.getElementById("nedThom").value);
	...
	var message = "Willst du diese Ressourcen aufbrauchen, um ein Schiff Typ "+type+" zu bauen?\n\n";

	message += nedWald+" Waldemarium\n";
	message += nedHelg+" Helgen\n";
	message += nedFran+" Frankur\n";
	message += nedMich+" Michaelum\n";
	message += nedJirk+" Jirkan\n";
	message += nedThom+" Thomasium";

	if(confirm(message)){}
	else { return; }
	
	if(waldCheck && helgCheck && franCheck && michCheck && jirkCheck && thomCheck)
	{
		$.post("schiffsbau.php", {"typ": type, "basis_id": baseID});
		alert('Schiff wird gebaut!');
		fillHTML("Shipcenter");
		return;
	}
	else
...
function makeFriend()
{
	...
	for(var i = 0; i < politicList.length; i++)
	{
		var docElem = document.getElementById('idPlayer_'+politicList[i].attributes.player_id.value);
		if(docElem.checked)
		{
			player = politicList[i].attributes.player_id.value;
			break;
		}
	}
	...
	if(player == -1)
	{
		alert("Du musst ein Spieler auswählen!");
		fillHTML("Alliances");
		return;
	}
	
	$.post("makeFriend.php", {"player_ID":player});
	alert('Der spieler '+player+'ist dein Frend!');
}
...
function makeEnemy()
{
	...
	$.post("makeEnemy.php", {"player_ID":player});
	alert('Der spieler '+player+'ist dein Feind!');
}
...
function makeNeutral()
{
	...
	$.post("makeNeutral.php", {"player_ID":player});
	alert('Der spieler '+player+'ist Neutral!');
}
...
function giveShip()
{
	...
	var politicList = xmlDoc.getElementsByTagName("politics")[0];
	if(typeof(politicList) === 'undefined')
	{
		alert("Du hast kein Freunden! :(");
		return;
	}
	else
	{
		var politics = politicList.getElementsByTagName("politic");
		var box = document.getElementById("friendSelect");
		for(var i = 0; i < politics.length; i++)
		{
			var pol = politics[i];
			if(pol.attributes.status.value == "1")
				box.innerHTML += '<option value="'+pol.attributes.player_id.value+'">Spieler '+pol.attributes.player_id.value+'</option>';
		}
		
		toGive = selected.attributes.id.value;
		
		$( "#dialog-form" ).dialog( "open" );
	}
}

function giveShipFinal(fID)
{
	$.post("giveShip.php", { 'fID': fID, 'ship': toGive } );
	alert('Schiff gesendet!');
}